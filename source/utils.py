...
def _load_rare_2024_samples(designated_class='TDE', input_path = None): # 2024 data already in the full data path. 
    if input_path is None:
        input_path = CSV_2024_PATH
...
def load_sample_lc(default_path = PHOTO_OUTPUT_PATH):
    ...
    if os.path.exists(default_path):
        ...
        print(f'No light curve samples found in {default_path}')
        return None
...
def load_sample_imgs(default_path = IMG_OUTPUT_PATH):
    ...
    '''
    if EXCEPTION_IMG_PATH is not None and os.path.exists(EXCEPTION_IMG_PATH):
        with open(EXCEPTION_IMG_PATH, 'r') as f:
            failed_img_ids = set([line.strip() for line in f])
    else:
        failed_img_ids = set()
    ...
    if os.path.exists(default_path):
        ...
        print(f'Loaded {len(img_list)} image samples from {default_path}')
        return img_list
    else:
        print(f'No image samples found in {default_path}')
        return None
...
def save_to_h5py(imageset, metaset, labels, idx_set, filepath):
    ...
    imageset = np.asarray(imageset, dtype=np.float32)
    metaset = np.asarray(metaset, dtype=np.float32)
    labels = np.asarray(labels, dtype=np.int32)
    idx_set = np.asarray(idx_set, dtype=np.int32)
    ...
    try:
        with h5py.File(filepath, "w") as f:
            f.create_dataset("imageset", imageset.shape, dtype=np.float32, data=imageset)
            f.create_dataset("metaset", metaset.shape, dtype=np.float32, data=metaset)
            f.create_dataset("labels", labels.shape, dtype=np.int32, data=labels)
            f.create_dataset("idx_set", idx_set.shape, dtype=np.int32, data=idx_set)
...
def open_with_h5py(filepath):
    ...
    with h5py.File(filepath, mode='r') as f:
        imageset = np.array(f['imageset'])
        try:
            labels = np.array(f['labels']).astype(np.int32)
        except:
            labels = np.array(f['label']).astype(np.int32)
        metaset = np.array(f['metaset'])
        idx_set = np.array(f['idx_set']).astype(np.int32)
